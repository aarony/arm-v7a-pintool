...
LOCALFUN VOID Handler(CONTROL_EVENT ev, VOID *val, CONTEXT *d1, VOID *d2, VOID *d3)
{
    switch(ev)
    {
      case CONTROL_START:
...
VOID Trace(TRACE trace, VOID *v)
{
    ...
    if (it == rtn_table.end()) 
    {
       RTN_TABLE_ENTRY *rtn_table_entry =  new RTN_TABLE_ENTRY(rtn_address, strdup(rtn_name));
